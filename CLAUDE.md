...
- `+100` per completed exam (`EXAM_BASE_SCORE` from `config.py`)
...
> **Stub note**: `scoring_manager.py` currently hardcodes `self.score += 10` in `exam_completed()`. Replace with `EXAM_BASE_SCORE` imported from `config.py` when implementing.
